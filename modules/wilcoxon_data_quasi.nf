process WILCOXON_DATA_QUASI{
  ...
  ## Re-run wilcoxon for our new dataset
  # Normalize counts
  countsframe.dge = DGEList(counts=countsframe.sub,group=samplesheet.sub\$phenotype)
  ...
  refcounts = select(countsframe.dge, subset(samplesheet.sub, phenotype == reflevel)\$sample)
  altcounts = select(countsframe.dge, subset(samplesheet.sub, phenotype != reflevel)\$sample)
  ...
  truedegs = row.names(subset(wilcox.padj,padj<$params.truecutoff))
