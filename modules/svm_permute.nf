process SVM_PERMUTE{
  ...
  output:
  path "svm_table.csv", emit: outfile
  path "nDEGs.txt", emit: nDEGs
  path "svm_permute_fs_*.png"
  ...
  ggsave(hockeyplot,
         filename = paste0("svm_permute_fs_",$x,".png"),
         device = "png", path = "./",
         width =  38, height = 23, units = "cm")
  
  nDEGs = length(which(outframe\$DEGstatus == TRUE))
  write.table(nDEGs, row.names = F, col.names = F, "nDEGs.txt")
